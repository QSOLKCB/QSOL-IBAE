...
29. Preserve the v0.5 authority path: the OpenAI supervisor may request, deterministic governance may grant or deny, and Rust may apply only the exact canonical governance-issued grant with its non-constructible in-process capability. Canonical hash consistency alone is not issuance. A runtime, tool backend, worker, scheduler, or benchmark cannot grant or enlarge continuation authority.
30. Progress must come from the policy-bound exact progress contract over canonical obligations or governed observed/derived counters, and orchestration rebinds must preserve the actual prior/current endpoints. Activity, model confidence, token counts, elapsed time, new information, regression, and strategy paraphrase do not independently authorize continuation.
31. A strategy change used for bounded recovery must have a new admitted strategy identity and structured material difference; it must bind valid capabilities/targets and fail closed when it reproduces bound cycle evidence. Strategy change is not itself progress.
...
34. `IBAE-CONTINUATION-CHECKPOINT-V1` supports exact structural in-process lineage only, including the live progress endpoint and recovery counters. Do not claim producer authentication, durable attestation, record-only authority reconstruction, or cross-process native resume. Semantic partial reasons must match the actual last denial, and watchdog observations must bind the exact live orchestration/runtime context; neither can establish completion or manufacture lease exhaustion.
