...
29. Preserve the v0.5 authority path: the OpenAI supervisor may request, deterministic governance may grant or deny, and Rust may apply only the exact canonical grant. A runtime, tool backend, worker, scheduler, or benchmark cannot grant or enlarge continuation authority.
30. Progress must come from the declared exact progress contract over canonical obligations or governed observed/derived counters. Activity, model confidence, token counts, elapsed time, new information, regression, and strategy paraphrase do not independently authorize continuation.
...
34. `IBAE-CONTINUATION-CHECKPOINT-V1` supports exact structural in-process lineage only. Do not claim producer authentication, durable attestation, record-only authority reconstruction, or cross-process native resume. Watchdog observations cannot establish completion or lease exhaustion.
